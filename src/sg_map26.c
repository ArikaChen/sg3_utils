                    ...
                    int follow_symlink, int verbose)
{
	...
	printf("called list_matching_nodes: %s %d:%d\n", dir_name, majj, minn);
       ...
       int result, int follow_symlink, int verbose)
{
	...
	printf("map sd: before if_directory_chdir %s\n", name);
       ...
       int result, int follow_symlink, int verbose)
{
	...
	printf("before if_directory_chdir %s\n", name);
        if (! if_directory_chdir(name, "device")) {
                fprintf(stderr, "sysfs problem with device: %s\n",
                        device_name);
                return 1;
        }
	printf("before if_directory_chdir 2: %s\n", from_sg.name);
        if ((1 == from_sg_scan(".", verbose)) &&
            (if_directory_chdir(".", from_sg.name))) {
		printf("after if_directory_chdir 2: %s\n", from_sg.name);
